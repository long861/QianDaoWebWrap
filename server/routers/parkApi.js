...
router.post('/createNews', function (req, res, next) {
  ...
  if (!_id) {
    ...
    var newNews = new modelsBox.GameNews(data);
    ...
    modelsBox.GameNews.findOne({ _id, state: 0, createBy }).then((News) => {
      if (!News) {
      ...
      modelsBox.GameNews.findOneAndUpdate({ _id, state: 0, from: 3, createBy }, { $set: data }, { multi: true }).then((News1) => {
        res.json({ code: 200 });
      })
    }).catch((err) => {
      res.json({ code: 500, "err": err, "message": "系统错误" })
...
router.post('/getParkNews', function (req, res, next) {
  ...
  if (!tags) {
    var allNews = () => {
      return modelsBox.GameNews.find({ state: 0, type: "2", from: 3 }).sort({ createtime: -1 }).exec();
    }
  } else {
    var allNews = () => {
      return modelsBox.GameNews.find({ tags: new RegExp(tags), state: 0, type: "2", from: 3}).sort({ createtime: -1 }).exec();
    }
  }
  var LimitNews = (NewsBox, page, limit) => {
    var newsArr = [];
...
router.post('/NewsInfoById', (req, res, next) => {
  ...
  if (!req.$user) return res.json({ code: 1050, message: '账号已失效，请重新登录' });
  const { _id } = req.body;
  modelsBox.GameNews.findOne({ _id, state: 0, from: 3 }).then((News) => {
    if (!News) {
...
router.post('/getKnowLByTags', (req, res, next) => {
  ...
  var allNews = () => {
    return modelsBox.GameNews.find({ tags, state: 0, type: "1", from: 3 }).sort({ createtime: -1 }).exec();
  }
  var LimitNews = (NewsBox, page, limit) => {
    var newsArr = [];
    for (var i = (page - 1) * limit; i < page * limit; i++) {
...
router.post('/getNewsByTags', (req, res, next) => {
  ...
  const { tags, page, limit } = req.body;
  var allNews = () => {
    return modelsBox.GameNews.find({ tags: new RegExp(tags), state: 0, type: "2", from: 3, createBy, }).sort({ createtime: -1 }).exec();
  }
  var LimitNews = (NewsBox, page, limit) => {
    var newsArr = [];
    for (var i = (page - 1) * limit; i < page * limit; i++) {
