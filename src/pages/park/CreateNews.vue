  ...
  <div class="app-container" v-if="isRealy">
    <div class="crumbs">
      <el-breadcrumb separator="/" v-if="!editNews">
        <el-breadcrumb-item>创建动态</el-breadcrumb-item>
      </el-breadcrumb>
    ...
    <div class="container">
      <div class="form-box">
        <el-form :model="newsInfo" label-width="120px" ref="newsInfo" :rules="rules">
          ...
          <el-form-item label="动态内容" prop="content" label-width="120px">
            <editor
              class="editor"
          ...
          >
            <el-input v-model="tags" auto-complete="off" placeholder="请输入类别，以”,“分割"></el-input>
          </el-form-item>
          <el-form-item>
...
export default {
  ...
  methods: {
    ...
    createNew() {
      ...
      data.tags = tagArr;
      api.post("/api/park/createNews", data).then(res => {
        if (res.data.code == 1050) {
          ...
          if (!this.editNews) {
            ...
            setTimeout(() => {
              this.$router.push({ path: "/park/news", name: "园区动态" });
            }, 1000);
          } else {
            this.$message({
              message: "编辑成功",
